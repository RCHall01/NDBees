...
Options:
    ...
    --headcrop 0 : trim bases off the beginning of each read
...
if ($croplength) {
    $minlength = $croplength;
    $maxlength = $croplength;
}
if (($read eq "r1") and ($headcrop) and ($r1enzyme ne "none")) {
    die "The --enzyme1 must be 'none' when using the --headcrop option";
}
if (($read eq "r2") and ($headcrop) and ($r2enzyme ne "none")) {
    die "The --enzyme2 must be 'none' when using the --headcrop option";
}
...
#    $r2enzyme = $r1enzyme unless ($r2enzyme);
    ...
    $r1overhang = $enzymes{$r1enzyme}{overhang} unless ($r1overhang);
    $r2overhang = $enzymes{$r2enzyme}{overhang} unless ($r2overhang);
    if ($read eq "r1") {
	$r1padding = $padding{"$r1enzyme-r1"} unless ($r1padding);
	$r2padding = $padding{"$r2enzyme-r2"} unless ($r2padding);
    } else {
	$r1padding = $padding{"$r2enzyme-r2"} unless ($r1padding);
	$r2padding = $padding{"$r1enzyme-r1"} unless ($r2padding);
    }
    if ($read eq "r2") {
	($r1enzyme, $r2enzyme) = ($r2enzyme, $r1enzyme);
	($r1overhang, $r2overhang) = ($r2overhang, $r1overhang);
    }
...
print "r1padding: $r1padding\n";
print "r2padding: $r2padding\n";
print "r1overhang: $r1overhang\n";
print "r2overhang: $r2overhang\n";
...
if (1) {
    ...
    print "Cropping $headcrop bases from 5' end\n" if ($headcrop);
...
"nlaiii" => {
name => "NlaIII",sequence => "CATG|",overhang => "CATG",},
"none" => {
name => "none",sequence => "|",overhang => "",},
"msei" => {
name => "MseI", sequence => "T|TAA", overhang => "TAA",},
"ecori" => {
name => "EcoRI",sequence => "G|AATTC",overhang => "AATTC",},
"nlaiii" => {
name => "NlaIII",sequence => "CATG|",overhang => "CATG",},
"mluci" => {
name => "MluCI",sequence => "|AATT",overhang => "AATT",},
);
...
$gatc_r2 = "C,CG,TAG,GTCA,CAAGT,TACGAA,CACTCTG,AGTACGGT,GTTCGACAT,TCGATGTGCT";
...
#$cg_r2 = ",T,GA,GTA,AGCTC,TATAAA,TTCATA";
$cg_r2 = ",T,GA,GTA,AGCT,TATAAA,CTTCATA,GC,ACATT";

$catc_r1 = "A,TG,CAG,GCTA,ACAGA,CTCCGA,GAATCGA,AGGTCCAA,TCACGCTCA";
$catc_r2 = ",";

$aatt_r1 = ",G,AC,TGG,CTGG,TGCCG";
$aatt_r2 = ",";
