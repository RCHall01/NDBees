...
###bring in data ####
#reference genome 
gff <- import("data/ncbi_dataset/ncbi_dataset/data/GCF_024542735.1/genomic.gff")
...
#snp list 
snp_data <- read.csv("data_raw/BOTE_RDA_cand.csv")
snp_data <- snp_data[,-c(5:23)]
snp_data <- na.omit(snp_data)
...
write.csv(df, "data/BOTE_RDAGOresults.csv")


###now again for LFMM
#snp list 
snp_data <- read.csv("data_raw/BOTE_LFMM_cand.csv")
...
write.csv(df, "data/BOTE_LFMMGOresults.csv")

###now again for Fst
#snp list 
snp_data <- read.csv("data_raw/BOTE_OutFlank_cand.csv")
#remove shared 
...
write.csv(df, "data/BOTE_OutFlankGOresults.csv")
